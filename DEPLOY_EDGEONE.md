...
| `EDGEONE_SECRET_ID` | EdgeOne API SecretId（可选，用于 CDN 缓存清除） | `your_secret_id` |
| `EDGEONE_SECRET_KEY` | EdgeOne API SecretKey（可选，用于 CDN 缓存清除） | `your_secret_key` |
| `EDGEONE_ZONE_ID` | EdgeOne 站点 ID（可选，用于 CDN 缓存清除） | `your_zone_id` |
...
如果上传、删除或修改文件后，CDN 缓存未及时清除，可以：

1. 在管理界面的"其他设置"中配置 EdgeOne API 凭证
2. 确保已正确配置 `EDGEONE_SECRET_ID`、`EDGEONE_SECRET_KEY` 和 `EDGEONE_ZONE_ID`
3. 或者手动在 EdgeOne 控制台中清除指定 URL 的缓存
